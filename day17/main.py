...
def p1():
    ...
    visited = {(0,0): 0} # map keeping track of min val of heat incurred to get to point[r][c]
    ...
    while len(q) != 0:
        ...
        for _ in range(size):
            ...
            if curr_point not in visited:
                visited[curr_point] = s.heat_loss
            else:
                visited[curr_point] = min(s.heat_loss, visited[curr_point])
            for dir, ch in zip(dirs, cardinal_l):
                if dir == s.dir and s.num_steps == 3:
                    continue
                next_r, next_c = s.r + dir[0], s.c + dir[1]
                if next_r >= 0 and next_r < m and next_c >= 0 and next_c < n:
                    next_incurred_hl = int(input[next_r][next_c])
                    if (next_r, next_c) in visited and visited[(next_r), (next_c)] < next_incurred_hl + s.heat_loss:
                        continue 
                    else:
                        if dir == s.dir:
